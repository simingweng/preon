...
import nl.flotsam.pecia.Table6Cols;
import nl.flotsam.pecia.builder.DocumentBuilder;
import nl.flotsam.pecia.builder.Table6ColsBuilder;

public class DefaultTable6ColsBuilder<T> extends AbstractTableBuilder<T>
        implements Table6ColsBuilder<T> {

    public DefaultTable6ColsBuilder(DocumentBuilder builder, T parent,
            LifecycleListener listener) {
        super(builder, parent, listener);
    }

    public Row<Entry<Entry<Entry<Entry<Entry<LastEntry<Table6Cols<T>>>>>>>> header() {
        return RowFactory.start((Table6Cols<T>) this, getBuilder(), true).col()
                .col().col().col().col().create(proceed(true));
    }

    public Row<Entry<Entry<Entry<Entry<Entry<LastEntry<Table6Cols<T>>>>>>>> row() {
        return RowFactory.start((Table6Cols<T>) this, getBuilder(), false)
                .col().col().col().col().col().create(proceed(false));
    }
